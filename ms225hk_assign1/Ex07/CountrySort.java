...
public class CountrySort {
    ...
    private static String[] COUNTRIES = {
            ...
            "Iceland", "India", "Indonesia", "Iran", "Iraq", "Ireland", "Palestine", "Italy", "Jamaica",
    ...
    public static void main(String[] args) {
        ...
        Comparator<String> reverseAlphabetic = (s1, s2) -> s2.charAt(0) - s1.charAt(0);      // compare ascii code
        Arrays.sort(COUNTRIES, reverseAlphabetic);
        System.out.println("\nCountries in reverse alphabetic order");
        for (int i = 0; i < 10; i++)
            System.out.println("\t" + COUNTRIES[i]);

        // Task 2: Print Top 10 list of countries with shortest names
        Comparator<String> shorestName = (s1, s2) -> s1.length() - s2.length();       // Must be updated
        Arrays.sort(COUNTRIES, shorestName);
        ...
        Comparator<String> countAs = (s1, s2) -> (int) (s2.chars().filter((alphapet) -> alphapet == 'A' ||
                alphapet == 'a').count() - s1.chars().filter((alphapet) -> alphapet == 'A' || alphapet == 'a').count());
